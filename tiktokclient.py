...
client: TikTokLiveClient = TikTokLiveClient(unique_id="@staff.play")
...
async def on_like(event: LikeEvent):
    print(f"@{event.user.unique_id} liked the stream!", flush=True)
    async with lock:
        command = "likex34"
        send_command_to_pyboy(command)
...
async def on_gift(event: GiftEvent):
    ...
    if event.gift.streakable and not event.gift.streaking:
        print(
            f'Thank you {event.user.unique_id}! They sent {event.gift.count}x "{event.gift.info.name}"',
            flush=True,
        )
    ...
    # Non-streakable gift
        ...
        print(
            f'Thank you {event.user.unique_id}! They sent "{event.gift.info.name}"',
            flush=True,
        )
...
async def on_join(event: JoinEvent):
    print(
        f"Hi and welcome, @{event.user.unique_id}! Thank you for joining the stream!",
        flush=True,
    )
