...
def process_command(command, pyboy_instance):
    ...
    try:
        logging.debug("Command received: %s", command, flush=True)
...
def comms():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        s.listen()
        logging.info("PyBoy Server started, waiting for commands...", flush=True)
...
def signal_handler(sig, frame):
    logging.info("Exiting...", flush=True)
    exit_event.set()
    sys.exit(0)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python server_pyboy.py <path_to_rom>", flush=True)
        sys.exit(1)
